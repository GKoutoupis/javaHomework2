...
public class MainApp {
   public static void main(String[] args) {
      ...
      Enoikiasi dvd2 = new Dvd("blue-ray", "finos film", "1995", "BR", 1, 5, "onomaPelati", 6971234567L, 150, 45, 3.7,
            4.5, "komodia", 0.0, "aaaa", "aaa", "aaa", 120);
      Enoikiasi dvd3 = new Dvd("dvd", "lucasarts", "1977", "Star Wars", 7, 5, "onomaPelati", 6998979695L, 7, 1411, 3.7,
            1, "sci-fi", 0.0, "Lucas", "Lucas", "Carrie Fisher,Mark Hamill,Harrison Ford", 121);
      Enoikiasi dvd4 = new Dvd("blue-ray", "New Line Cinema", "2003", "Return of the King", 1, 5, "onomaPelati",
            698988878685L, 7, 208, 3.7, 4.5, "adventure,sci-fi", 0.0, "Peter Jackson", "J.R.R Tolkien",
      ...
      Enoikiasi game4 = new Game("Playstation", "Bethesda", "2011", "Skyrim", 7, 202, "onomaPelati", 6949484746L, 60,
            216, 2.5, 3.0, "RPG,Adventure", 0.0);
      Enoikiasi game5 = new Game("Xbox", "Microsoft", "2001", "Halo", 7, 4, "onomaPelati", 6939383736L, 7, 34, 2.5, 3.0,
            "Shooting,Adventure", 0.0);
      Enoikiasi game6 = new Game("Xbox", "Microsoft", "2018", "Sea of Thieves", 7, 18, "onomaPelati", 6929282726L, 7,
            45, 2.5, 3.0, "MMO", 0.0);
      ...
      data.ShowAll();
      // prepei na uparxoun hdh ta products1.txt kai products2.txt
      // CreateFileProsEnoik(data); prepei na
      // CreateFileEnoikiaseis(data);
      loadprosenoik(data);
      loadnoikiasmena(data);
            ...
            if (data.booleanfinder(name, platforma)) {
               ...
               if (answer.equals("y")) {
                  ...
                  if (diathesimatemaxia > 0) {
                     ...
                     data.noikiasmena.get(tmp1).setTelikoKostos(
                           kostosenoik + (hmeres - data.prosenoik.get(pos).getXronosenoikiasis()) * extrakostos);
   ...
   public static void ShowChoices() {
      ...
      System.out.println("3. Exit & Dimiourgia txt ");
   }
   ...
   public static void SetXronoEnoik(String platforma, Integer pos, Database data) {
      if (platforma.equals("blue-ray")
            || (platforma.equals("dvd")) && data.prosenoik.get(pos).getEtosparagvghs() == "2019") {
         data.prosenoik.get(pos).setXronosEnoikiasis(1);
      } else {
   ...
   public static void CreateFileProsEnoik(Database data)// write data from list to file
   {
      ...
      try {
         writer = new FileWriter(new File("products1.txt"));
   ...
   public static void CreateFileEnoikiaseis(Database data)// write data from list to file
   {
      ...
      try {
         writer = new FileWriter(new File("products2.txt"));
         ...
         for (Enoikiasi item : data.noikiasmena) {
            if (item instanceof Dvd) {
               ...
               writer.write("\t" + "RENTAL" + "\n" + "\t" + "{" + "\n" + "\t" + "\t" + "ITEM_TYPE " + "game" + "\n"
                     ...
                     + ((Game) item).getHmeromhniaEnoikiasis() + "\n" + "\t" + "\t" + "COMPANY "
                     + ((Game) item).getEtairia() + "\n" + "\t" + "}" + "\n");
            }
   ...
   public static void loadprosenoik(Database data) {
      ...
      try {
         ...
         reader = new BufferedReader(new FileReader(new File("products1.txt")));
               ...
               if (line.trim().equals("ITEM")) {
                  ...
                  if (line.trim().equals("{")) {
                     ...
                     if (line.trim().startsWith("ITEM_TYPE ")) {
                        line = reader.readLine();
                        if (line.trim().substring(10).trim().equals("DVD")) {
                           Dvd product = new Dvd("","","","",0,0,"",0L,0,0,0.0,0.0,"",0.0,"","","",0);
                           line = reader.readLine();
                           if (line.trim().startsWith("SUB_TYPE "))
                              product.setPlatforma(line.substring(9).trim()); 
                           line = reader.readLine();
                           if (line.trim().startsWith("TITLE "))
                              product.setTitlos(line.substring(6).trim());
                           line = reader.readLine();
                           if (line.trim().startsWith("YEAR "))
                              product.setEtosParagvghs(line.substring(5).trim());
                           line = reader.readLine();
                           if (line.trim().startsWith("CAST "))
                              product.setIthopoioi(line.substring(5).trim());
                           line = reader.readLine();
                           if (line.trim().startsWith("CATEGORY "))
                              product.setKatigoria(line.substring(9).trim());
                           line = reader.readLine();
                           if (line.trim().startsWith("COPIES "))
                              product.setTemaxia(Integer.parseInt(line.substring(7).trim()));
                           line = reader.readLine();
                           if (line.trim().equals("}"))
                              data.prosenoik.add(product);
                        } else if (line.trim().substring(10).trim().equals("game")) {
                           Game product = new Game("","","","",0,0,"",0L,0,0,0.0,0.0,"",0.0);
                           line = reader.readLine();
                           if (line.trim().startsWith("SUB_TYPE "))
                              product.setPlatforma(line.substring(9).trim());
                           line = reader.readLine();
                           ...
                           if (line.trim().startsWith("COMPANY "))
                              product.setTitlos(line.substring(8).trim());
                           line = reader.readLine();
                           if (line.trim().startsWith("COPIES "))
                              product.setTemaxia(Integer.parseInt(line.substring(7).trim()));
                           line = reader.readLine();
   ...
   public static void loadnoikiasmena(Database data) {
